...
class EventCrawler:
    ...
    def scrape_event(self):
        ...
        for row in rows:
            try:
                ...
                if key == "개최기간":
                    start_date, end_date, start_time, end_time = self.parse_period(value)
                details[key] = value
            except Exception:
                continue
        ...
        INSERT INTO events (
            ...
            age_restriction, event_inquiry, event_description
        ) VALUES (
            %s, %s, %s, %s,
            %s, %s,
            %s, %s,
    ...
    """
    def parse_period(self, period_text : str):
        try:
            # 예시: "2025. 08.14 ~ 08.16 : 10:00~21:00"
            # 1) "YYYY.MM.DD ~ YYYY.MM.DD" 형태인지 먼저 확인
            full_date_match = re.search(r"(\d{4}\.\s*\d{2}\.\d{2})\s*~\s*(\d{4}\.\s*\d{2}\.\d{2})", period_text)
            if full_date_match:
                start_date_str = full_date_match.group(1).replace(" ", "")
                end_date_str = full_date_match.group(2).replace(" ", "")
                start_date = datetime.strptime(start_date_str, "%Y.%m.%d").date()
                end_date = datetime.strptime(end_date_str, "%Y.%m.%d").date()
            else:
                # 2) "YYYY.MM.DD ~ MM.DD" 형태
